...
bed_positions_plant0 = identify_positions(terrain_layers[0], colors["blue"])
bed_positions_plant1 = identify_positions(terrain_layers[1], colors["blue"])

# Identificar todas las posiciones de los puestos de trabajo (grises)
work_positions_plant0 = identify_positions(terrain_layers[0], colors["gray"])
work_positions_plant1 = identify_positions(terrain_layers[1], colors["gray"])

# Identificar todas las posiciones de comida (amarillo)
food_positions_plant0 = identify_positions(terrain_layers[0], colors["yellow"])
food_positions_plant1 = identify_positions(terrain_layers[1], colors["yellow"])
...
# Concatenar camas y posiciones de trabajo en ambas plantas
bed_positions = [(0, pos) for pos in bed_positions_plant0] + [(1, pos) for pos in bed_positions_plant1]
work_positions = [(0, pos) for pos in work_positions_plant0] + [(1, pos) for pos in work_positions_plant1]
food_positions = [(0, pos) for pos in food_positions_plant0] + [(1, pos) for pos in food_positions_plant1]

for _ in range(num_agents):
    # Inicializar en una planta aleatoria
    if np.random.rand() < 0.5:
        initial_position = walkable_positions_plant0[np.random.choice(len(walkable_positions_plant0))]
        initial_floor = 0
    else:
        initial_position = walkable_positions_plant1[np.random.choice(len(walkable_positions_plant1))]
        initial_floor = 1

    agent_positions.append(initial_position)
    agent_floors.append(initial_floor)

    # Asignar una cama aleatoria
    bed_position = bed_positions[np.random.choice(len(bed_positions))]
    agent_beds.append(bed_position)

    # Asignar un puesto de trabajo aleatorio
    work_position = work_positions[np.random.choice(len(work_positions))]
    agent_workplaces.append(work_position)

    # Asignar la posición de comida más cercana a la cama asignada
    if bed_position[0] == 0:
        distances_to_food = np.linalg.norm(food_positions_plant0 - bed_position[1], axis=1)
        nearest_food_index = np.argmin(distances_to_food)
        nearest_food_position = food_positions_plant0[nearest_food_index]
        agent_food_positions.append((0, tuple(nearest_food_position * scale_factor)))
    else:
        distances_to_food = np.linalg.norm(food_positions_plant1 - bed_position[1], axis=1)
        nearest_food_index = np.argmin(distances_to_food)
        nearest_food_position = food_positions_plant1[nearest_food_index]
        agent_food_positions.append((1, tuple(nearest_food_position * scale_factor)))
...
# Función de heurística para A* en 3D (considerando plantas)
...
def a_star_3d(terrain_layers, start, goal, scale_factor):
    ...
    directions_2d = [(0, scale_factor), (0, -scale_factor), (scale_factor, 0), (-scale_factor, 0)]
    ...
    while open_list:
        ...
        # Moverse en el mismo piso
        for direction in directions_2d:
            neighbor = (current[0], current[1] + direction[0], current[2] + direction[1])
            if 0 <= neighbor[1] < terrain_layers[current[0]].shape[0] and 0 <= neighbor[2] < terrain_layers[current[0]].shape[1]:
                current_color = terrain_layers[current[0]][neighbor[1], neighbor[2]].tolist()
                if current_color in [list(colors["green"]), list(colors["yellow"]), list(colors["blue"]), list(colors["red"]), list(colors["cyan"]), list(colors["gray"]), list(colors["dark_green"]), list(colors["elevator"])]:
                    ...
                    if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                        ...
                        f_score[neighbor] = tentative_g_score + heuristic_3d(neighbor, goal)
                        heapq.heappush(open_list, (f_score[neighbor], neighbor))
        
        # Moverse verticalmente entre pisos usando ascensores
        if current[0] == 0:  # Si estamos en la planta 0
            for elevator in elevator_positions_plant0:
                if (current[1], current[2]) == tuple(elevator):
                    neighbor = (1, elevator[0], elevator[1])  # Moverse a la planta 1
                    tentative_g_score = g_score[current] + 1
                    if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                        came_from[neighbor] = current
                        g_score[neighbor] = tentative_g_score
                        f_score[neighbor] = tentative_g_score + heuristic_3d(neighbor, goal)
                        heapq.heappush(open_list, (f_score[neighbor], neighbor))
        elif current[0] == 1:  # Si estamos en la planta 1
            for elevator in elevator_positions_plant1:
                if (current[1], current[2]) == tuple(elevator):
                    neighbor = (0, elevator[0], elevator[1])  # Moverse a la planta 0
                    tentative_g_score = g_score[current] + 1
                    if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                        came_from[neighbor] = current
                        g_score[neighbor] = tentative_g_score
...
def move_agents(agent_states, terrain_layers, scale_factor, steps=10):
    ...
    for step in range(steps):
        # Reiniciar las capas de agentes para cada planta
        agents_layers[0].fill(0)
        agents_layers[1].fill(0)
        ...
        for i in range(len(agent_states["positions"])):
                ...
                if need == "work":
                    ...
                    if np.random.rand() < 0.5 and len(park_positions) > 0:
                        ...
                        if len(resting_positions) > 0:
                            distances = [heuristic_3d((floor, x, y), (floor, pos[0], pos[1])) for pos in resting_positions]
                            nearest_index = np.argmin(distances)
            ...
            if agent_states["targets"][i] is not None and np.array_equal((x, y), agent_states["targets"][i]) and floor == target_floor:
                agent_states["paths"][i] = None
                agent_states["targets"][i] = None

            if agent_states["paths"][i] is None or len(agent_states["paths"][i]) == 0:
                path = a_star_3d(terrain_layers, (floor, x, y), (target_floor, target_position[0], target_position[1]), scale_factor)
                agent_states["paths"][i] = path if path else []
                agent_states["targets"][i] = target_position
                agent_states["floors"][i] = target_floor  # Cambiar el agente a la planta de destino

            if agent_states["paths"][i]:
                points = np.array(agent_states["paths"][i], np.int32)
                points = points[:, [2, 1]] * scale_factor
            ...
            if agent_states["paths"][i]:
                ...
                agent_states["positions"][i] = (next_position[1], next_position[2])
                if need == "wc" and np.array_equal(next_position[1:], agent_states["targets"][i]):
                    agent_states["wc_timer"][i] = 60
                    agent_states["previous_need"][i] = need

            # Verificar que la posición esté dentro de los límites de la imagen antes de dibujar el agente
            if 0 <= x < agents_layers[floor].shape[0] and 0 <= y < agents_layers[floor].shape[1]:
                # Dibujar el agente en negro en la posición actual
                agents_layers[floor][x:x+scale_factor, y:y+scale_factor] = (0, 0, 0)

        # Actualizar visualización para cada planta
