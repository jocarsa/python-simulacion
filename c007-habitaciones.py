...
for i in range(num_viviendas):
    ...
    # Dibujar la línea transversal para la vivienda
    cv2.line(image, tuple(outer_point), tuple(inner_point), color=(255, 255, 255), thickness=2)
    
    # Calcular el número de habitaciones dentro de la vivienda
    num_habitaciones = random.randint(2, 4)
    
    # Dibujar divisiones internas (habitaciones) dentro de la vivienda
    for j in range(1, num_habitaciones):
        # Interpolación lineal entre los puntos exterior e interior
        new_outer_point = outer_point + (inner_point - outer_point) * (j / num_habitaciones)
        new_inner_point = inner_point + (outer_point - inner_point) * (j / num_habitaciones)
        
        # Dibujar la línea que divide la vivienda en habitaciones
        cv2.line(image, tuple(new_outer_point), tuple(new_inner_point), color=(255, 255, 255), thickness=1)

# Guardar la imagen resultante
cv2.imwrite('edificio_con_habitaciones.png', image)

# Mostrar la imagen
cv2.imshow('Building with Courtyard, Apartments, and Rooms', image)
cv2.waitKey(0)
