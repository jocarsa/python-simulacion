...
# Definir el horario base y las variaciones para cada agente
agent_schedule_variations = []
for i in range(num_agents):
    # Variación de hasta ±30 minutos (±1800 segundos)
    variation = {
        "rest_start": np.random.randint(-1800, 1800),
        "rest_end": np.random.randint(-1800, 1800),
        "breakfast_start": np.random.randint(-1800, 1800),
        "breakfast_end": np.random.randint(-1800, 1800),
        "work_start": np.random.randint(-1800, 1800),
        "work_end": np.random.randint(-1800, 1800),
        "lunch_start": np.random.randint(-1800, 1800),
        "lunch_end": np.random.randint(-1800, 1800),
        "afternoon_rest_start": np.random.randint(-1800, 1800),
        "afternoon_rest_end": np.random.randint(-1800, 1800),
        "dinner_start": np.random.randint(-1800, 1800),
        "dinner_end": np.random.randint(-1800, 1800),
        "night_rest_start": np.random.randint(-1800, 1800),
        "night_rest_end": np.random.randint(-1800, 1800)
    }
    agent_schedule_variations.append(variation)
...
def move_agents_with_variations(agent_states, agent_schedule_variations, simulation_map, scale_factor, steps=2400):
    ...
    for step in range(steps):
        ...
        for i in range(len(agent_states["positions"])):
            ...
            # Incorporar variaciones de horario
            rest_start = 22 * 3600 + agent_schedule_variations[i]["rest_start"]
            rest_end = 8 * 3600 + agent_schedule_variations[i]["rest_end"]
            breakfast_start = 8 * 3600 + agent_schedule_variations[i]["breakfast_start"]
            breakfast_end = 9 * 3600 + agent_schedule_variations[i]["breakfast_end"]
            work_start = 9 * 3600 + agent_schedule_variations[i]["work_start"]
            work_end = 13 * 3600 + agent_schedule_variations[i]["work_end"]
            lunch_start = 13 * 3600 + agent_schedule_variations[i]["lunch_start"]
            lunch_end = 14 * 3600 + agent_schedule_variations[i]["lunch_end"]
            afternoon_rest_start = 14 * 3600 + agent_schedule_variations[i]["afternoon_rest_start"]
            afternoon_rest_end = 20 * 3600 + agent_schedule_variations[i]["afternoon_rest_end"]
            dinner_start = 20 * 3600 + agent_schedule_variations[i]["dinner_start"]
            dinner_end = 21 * 3600 + agent_schedule_variations[i]["dinner_end"]
            night_rest_start = 21 * 3600 + agent_schedule_variations[i]["night_rest_start"]
            night_rest_end = 22 * 3600 + agent_schedule_variations[i]["night_rest_end"]

            # Establecer la necesidad según la hora y con las variaciones individuales
            if rest_start <= current_hour * 3600 or current_hour * 3600 < rest_end:
                need = "rest"
            elif breakfast_start <= current_hour * 3600 < breakfast_end:
                need = "food"
            elif work_start <= current_hour * 3600 < work_end:
                if days_of_week[day_of_week] in ["Saturday", "Sunday"]:
                    need = "resting"  # En fines de semana, los agentes descansan en lugar de trabajar
                else:
                    need = "work"
            elif lunch_start <= current_hour * 3600 < lunch_end:
                need = "food"
            elif afternoon_rest_start <= current_hour * 3600 < afternoon_rest_end:
                need = "resting"
            elif dinner_start <= current_hour * 3600 < dinner_end:
                need = "food"
            elif night_rest_start <= current_hour * 3600 < night_rest_end:
                need = "resting"
...
# Ejecutar la simulación con las variaciones de horario
move_agents_with_variations(agent_states, agent_schedule_variations, simulation_map, scale_factor, steps=2400000)
