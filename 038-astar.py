...
# Definir el factor de escala para la imagen grande
scale_factor = 8  # Escalamos por 8 para trabajar con sprites de 8x8
simulation_map = image.copy()  # Esta es la imagen que se utilizará para los cálculos
...
num_agents = 140  # Número de agentes
...
# Algoritmo A* para encontrar el camino en la imagen pequeña
    ...
    while open_list:
        ...
        for direction in directions:
            ...
            if 0 <= neighbor[0] < simulation_map.shape[0] and 0 <= neighbor[1] < simulation_map.shape[1]:
                ...
                if any([np.all(current_color == np.array(value)) for value in colors.values()]):
...
def move_agents(agent_states, simulation_map, scale_factor, sprites, steps=10):
    ...
    for step in range(steps):
        # Aquí NO modificamos directamente la imagen original `simulation_map`
