...
agent_positions = np.array([walkable_positions[np.random.choice(len(walkable_positions))] for _ in range(num_agents)]) * scale_factor
...
def move_agents(agent_states, simulation_map, scale_factor, steps=10):
    ...
    for step in range(steps):
        ...
        for i in range(len(agent_states["positions"])):
            ...
            if agent_states["paths"][i] is None or len(agent_states["paths"][i]) == 0:
                ...
                if len(target_positions) > 0:
                    ...
                    # Dibujar la polilínea del camino en una copia del mapa para no afectar el mapa principal
                    if path:
        ...
        temp_map = path_map.copy() if 'path_map' in locals() else simulation_map.copy()
...
print("Dimensiones de la imagen ampliada:", resized_image.shape)
print("Posiciones iniciales de los agentes:", agent_positions)
