...
def update_agents_on_map(background_image, sprites, scale_factor, previous_positions, json_path='agent_positions.json'):
    ...
    for step in range(steps):
        dynamic_layer = background_image.copy()  # Crear una copia limpia de la imagen de fondo para dibujar sobre ella
        ...
        cv2.imshow('Mapa con Agentes', dynamic_layer)
        if cv2.waitKey(30) & 0xFF == ord('q'):
            return dynamic_layer, previous_positions

    # Actualizar las posiciones anteriores con las nuevas posiciones
    for agent in agent_data.get("agents", []):
        agent_id = agent["id"]
        agent_position = agent["position"]
        x_new, y_new = int(agent_position[1]) * scale_factor, int(agent_position[0]) * scale_factor
    ...
    return background_image, new_positions

# Bucle para actualizar la imagen cada segundo
while True:
    # Actualizar la imagen con los agentes y animar el movimiento
    dynamic_image, previous_positions = update_agents_on_map(background_image, sprites, scale_factor, previous_positions)
