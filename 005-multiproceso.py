...
# Función para procesar cada agente en paralelo
def process_agent(i, agent_state, simulation_map, scale_factor):
    x, y = agent_state["positions"][i]
    need = agent_state["needs"][i]

    # Si no hay un camino calculado o el camino está agotado, calcula un nuevo camino
    if agent_state["paths"][i] is None or len(agent_state["paths"][i]) == 0:
        ...
        if len(target_positions) > 0:
            ...
            agent_state["paths"][i] = path if path else []  # Asignar camino o una lista vacía si no hay camino

    # Moverse al siguiente paso del camino calculado
    if agent_state["paths"][i]:
        next_position = agent_state["paths"][i].pop(0)
        agent_state["positions"][i] = next_position
    
    # Si alcanzó su objetivo o el camino se agotó, asignar una nueva necesidad aleatoria
    if len(agent_state["paths"][i]) == 0:
        agent_state["needs"][i] = np.random.choice(["food", "rest", "wc"])
        agent_state["paths"][i] = None  # Reiniciar el camino

    return agent_state

# Función para mover agentes según necesidades usando A* y multiprocessing
def move_agents_parallel(agent_states, simulation_map, scale_factor, steps=10):
    for step in range(steps):
        pool = mp.Pool(mp.cpu_count() // 2)  # Usa la mitad de los núcleos disponibles
        results = [pool.apply_async(process_agent, args=(i, agent_states, simulation_map, scale_factor)) for i in range(len(agent_states["positions"]))]
        updated_agent_states = {"positions": [], "needs": [], "paths": []}
        
        for r in results:
            result = r.get()
            updated_agent_states["positions"].append(result["positions"][i])
            updated_agent_states["needs"].append(result["needs"][i])
            updated_agent_states["paths"].append(result["paths"][i])
        
        pool.close()
        pool.join()

        agent_states = updated_agent_states  # Actualizar el estado de los agentes
...
move_agents_parallel(agent_states, simulation_map, scale_factor, steps=2400)  # Simulación de 2400 pasos
