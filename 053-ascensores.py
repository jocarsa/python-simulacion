...
colors = {
    ...
    "elevator": (0, 127, 255) # ascensor en formato BGR (equivalente a (255, 127, 0) en RGB)
}

# Identificar las posiciones de los ascensores
elevator_positions = np.argwhere(np.all(terrain_layers == colors["elevator"], axis=-1))
...
# Función de heurística mejorada para A* (distancia Euclidiana en 3D)
def heuristic(a, b):
    return np.sqrt((a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2 + (a[2] - b[2]) ** 2)

# Algoritmo A* mejorado para encontrar el camino en 3D con ascensores
def a_star_3d(terrain_layers, start, goal, scale_factor):
    ...
    while open_list:
        ...
        if any(np.array_equal(current, e) for e in elevator_positions):
            for dz in [-1, 1]:  # Solo puede moverse entre la planta 0 y 1
                ...
                if 0 <= neighbor[2] < terrain_layers.shape[2]:  # Verificar que no salga del rango de plantas
                    current_color = terrain_layers[neighbor[0], neighbor[1], neighbor[2]].tolist()
                    if current_color == list(colors["green"]) or current_color == list(colors["yellow"]) or current_color == list(colors["blue"]) or current_color == list(colors["red"]) or current_color == list(colors["cyan"]) or current_color == list(colors["gray"]) or current_color == list(colors["dark_green"]):
                        tentative_g_score = g_score[current] + 1  # Se mantiene el mismo coste para movimiento vertical
                        if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                            came_from[neighbor] = current
                            g_score[neighbor] = tentative_g_score
                            f_score[neighbor] = tentative_g_score + heuristic(neighbor, goal)
