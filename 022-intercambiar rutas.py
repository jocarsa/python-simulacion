...
def move_agents(agent_states, simulation_map, scale_factor, steps=10):
    ...
    cv2.resizeWindow(window_name, simulation_map.shape[1], simulation_map.shape[0])  # Ajustar la ventana al tamaño de la imagen original
    ...
    for step in range(steps):
        ...
        for i in range(len(agent_states["positions"])):
            ...
            if agent_states["targets"][i] and (x, y) == agent_states["targets"][i]:
                ...
                agent_states["needs"][i] = np.random.choice(["food", "rest", "wc", "resting"])
            ...
            if agent_states["paths"][i]:
                # Invertir X e Y solo al dibujar
                points = np.array(agent_states["paths"][i], np.int32)
                points = points[:, [1, 0]] * scale_factor  # Invertir X e Y al dibujar
