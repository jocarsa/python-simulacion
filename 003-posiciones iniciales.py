...
# Encontrar todas las posiciones verdes (terreno pisable)
green_positions = np.argwhere(np.all(simulation_map == colors["green"], axis=-1))

# Seleccionar posiciones iniciales aleatorias para los agentes solo sobre terreno pisable
initial_positions = green_positions[np.random.choice(len(green_positions), num_agents, replace=False)] * scale_factor
...
def move_agents(agent_states, simulation_map, scale_factor, steps=10):
    for step in range(steps):
        ...
        for pos in agent_states["positions"]:
            # Asegurarse de que los agentes sean visibles (dibujarlos con un tamaño adecuado)
