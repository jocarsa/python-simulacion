...
scale_factor = 4  # Cambié a 4 para evitar salirnos de los límites
        ...
        for i in range(len(agent_states["positions"])):
            ...
            if agent_states["paths"][i] is None or len(agent_states["paths"][i]) == 0:
                ...
                if len(target_positions) > 0:
                    ...
                    # Dibujar la polilínea del camino en la imagen
                    if path:
                        points = np.array(path, np.int32)
                        points = points.reshape((-1, 1, 2))
                        cv2.polylines(simulation_map, [points], isClosed=False, color=colors["orange"], thickness=2)
